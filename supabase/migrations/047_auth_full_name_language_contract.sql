...
BEGIN
  ...
  metadata_language := nullif(trim(COALESCE(NEW.raw_user_meta_data->>'language', '')), '');
  ...
  IF metadata_language NOT IN ('en-US', 'pt-BR') THEN
    metadata_language := 'en-US';
  END IF;

  INSERT INTO public.profiles (id, first_name, middle_name, last_name, username, email, language)
  VALUES (
    NEW.id,
    ...
    COALESCE(NEW.email, ''),
    COALESCE(metadata_language, 'en-US')
  )
